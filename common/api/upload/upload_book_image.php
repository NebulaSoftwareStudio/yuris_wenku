...
//设置目录
$dir = iconv("UTF-8", "GBK", "../../../upload/images/book/$user_account/");
//检查目录是否存在，不存在则创建目录
if (!file_exists($dir)) {
    mkdir($dir, 0777, true);
}
...
if ($_FILES["file"]["error"] > 0) {
    ...
    if(isset($_FILES["file"]["tmp_name"])&&$_FILES["file"]["tmp_name"]!=''){
        ...
        //保存文件到云端
        move_uploaded_file($_FILES["file"]["tmp_name"], "../../../upload/images/book/$user_account/".$time.$type);
        $url = "upload/images/book/$user_account/".$time.$type;
        //获取文件原始类型
